...
class OneBot11Channel(CustomChannel):
    ...
    def send_message(self, title: str, text: str, type: NotificationType = None, ext_info: dict = {}) -> bool:
        ...
        if not message_template:
            message_template = (self.config_default or {}).get("message_template")
        # 消息
        message = self.render_template(text=message_template, variables=template_variables)
        # 渲染结果为空时，使用标题和内容兜底
        if not message or not message.strip():
            fallback_parts = [part for part in [title, text] if part and str(part).strip()]
            message = "\n".join(fallback_parts)
        # 仍然为空则中止发送
        if not message or not message.strip():
            logger.warn(f"发送消息中止: channel = {self.comp_name}, type = {type_str}, 消息内容为空")
            return False
        # 支持OneBot-11消息段：渲染结果为JSON数组时按数组发送
